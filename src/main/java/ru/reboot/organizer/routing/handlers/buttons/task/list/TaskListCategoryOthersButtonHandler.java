...
public class TaskListCategoryOthersButtonHandler implements ButtonHandler {
    private final SessionManagerService sessionManagerService;
    private final TaskPaginationButtonHandler taskPaginationButtonHandler;

    @Override
    public ButtonType getHandledButton() {
        return ButtonType.TASK_LIST_CATEGORY_OTHERS;
    }

    @Override
    public UnifiedResponse handleButton(Long userId, String payload) {
        sessionManagerService.setUserScreen(userId, UserScreens.TASK_LIST_CATEGORY_OTHERS);

        return taskPaginationButtonHandler.handleButton(userId, ButtonType.TASK_LIST_PAGE_TASKS.getPayload() + "oth_0");
    }
}
